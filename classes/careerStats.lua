...
function CareerStats:init(previous)
	...
	local skillTextBox = nil
	local skillDTextBox
	if skill == "Unskilled" then
		skillTextBox = TextField.new(self.smallfont, "Skill: Unskilled")
		skillTextBox:setTextColor(0xffffff)
		skillTextBox:setPosition(0.5*WX0 - skillTextBox:getWidth()/2, 30)
		self:addChild(skillTextBox)
		
		skillDTextBox = TextField.new(self.xsmallfont, "No skill learned yet!")
	...
	local baseSkl = 3
	local baseDef = 3
	local points = careerTable["Points"]
	...
	if tonumber(careerTable["World"]) == 0 then
		...
		local function onColorChanged(e)
			...
			careerTable["R"] = e.r/25.5
			careerTable["G"] = e.g/25.5
			careerTable["B"] = e.b/25.5
		end
