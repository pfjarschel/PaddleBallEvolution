...
function MainMenu_Arena2:init(dataTable)
	...
	local selarenaTextBox = TextField.new(self.font, arena)
	...
	local arenaImg = Bitmap.new(Texture.new(arenaImgs[arenaIndex]))
	local pongfg = Bitmap.new(Texture.new("imgs/backs/pongbg.png"))
	pongfg:setScale(1,1)
	...
	self.prevBut:addEventListener(Event.TOUCHES_END, function(event)
		if self.prevBut:hitTestPoint(event.touch.x, event.touch.y) then
			...
			selarenaTextBox = TextField.new(self.font, arena)
			selarenaTextBox:setTextColor(0xffffff)
			selarenaTextBox:setPosition(192 - selarenaTextBox:getWidth()/2, 0.5*WY + 55)
			self:addChild(selarenaTextBox)
			...
			if arena == "Normal" then
				...
				arenaImg:lineTo(0, 0)
				arenaImg:endPath()
				pongfg = Bitmap.new(Texture.new("imgs/backs/pongbg.png"))
				pongfg:setScale(1,1)
				--pongfg:setAnchorPoint(0.5,0.5)
				pongfg:setScale(0.5,0.5)
				arenaImg:addChild(pongfg)
			...
			elseif arena == "Random" then
				...
				arenaImg:lineTo(0, 0)
				arenaImg:endPath()
				local giantfont = TTFont.new("fonts/anita.ttf", 240, false)
				...
				arenaImg = Bitmap.new(Texture.new(arenaImgs[arenaIndex]))
				pongfg = Bitmap.new(Texture.new("imgs/backs/pongbg.png"))
				pongfg:setScale(1,1)
				pongfg:setAnchorPoint(0.5,0.5)
				pongfg:setScale(0.5,0.5)
