...
function MainMenu_Arena:init()
	...
	if classAI == "Random" then
		classname = "Random"
	elseif classAI == "Custom" then
		classname = "Custom Class"
	else
		classname = classTable[classAI][11]
	end
	local selClassTextBoxAI = TextField.new(self.font, classname)
	...
	local selSkillTextBoxAI = TextWrap.new(classSkill[classIndexR]..": "..classSkillDesc[classIndexR], 300, "center", 7, self.smallfont)
	...
	self.prevButAI:addEventListener(Event.TOUCHES_END, function(event)
		if self.prevButAI:hitTestPoint(event.touch.x, event.touch.y) then
			...
			selClassTextBoxAI = TextField.new(self.font, classname)
			selClassTextBoxAI:setTextColor(0xffffff)
			selClassTextBoxAI:setPosition(WX - 192 - selClassTextBoxAI:getWidth()/2, 0.5*WY + 10)
			self:addChild(selClassTextBoxAI)
